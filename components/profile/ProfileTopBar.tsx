    ...
    <header className="bg-[#71C4FF]">
      <div className="mx-auto flex h-[83px] w-full max-w-[1440px] items-center justify-between px-4 sm:px-8 lg:px-16">
        <Link href="/" className="flex items-center gap-3 text-white">
          ...
          <span className="text-2xl font-semibold tracking-tight">BruinPlace</span>
        </Link>
                  ...
                  <div className="space-y-1">
                    <p
                      className={`text-sm leading-snug ${
                        notification.read ? "text-zinc-500" : "font-medium text-zinc-900"
                      }`}
                    >
                      {notification.text}
                    </p>
                    <p className="text-xs text-zinc-500">{notification.timeLabel}</p>
                  </div>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
