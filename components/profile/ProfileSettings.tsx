...
export default function ProfileSettings() {
  ...
  function handleFieldChange<K extends keyof ProfileForm>(field: K, value: ProfileForm[K]) {
    setDraftProfile((current) => ({ ...current, [field]: value }));
  }

  const profileValues = isEditingProfile ? draftProfile : profile;
  const avatarFallback = `${profile.firstName.charAt(0)}${profile.lastName.charAt(0)}`.toUpperCase();

  return (
    <ProfilePageShell activeTab="settings" avatarSrc={photoUrl} avatarFallback={avatarFallback}>
        <section className="rounded-[14px] border border-[#d8d8d8] bg-white p-6 shadow-[0px_2px_4px_rgba(0,0,0,0.08)]">
          <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
            <button type="button" onClick={handleStartEditing} className="inline-flex items-center gap-4 text-left">
              <Avatar className="h-[54px] w-[54px]">
                {photoUrl ? <AvatarImage src={photoUrl} alt="Profile photo" className="object-cover" /> : null}
